...
export type SoundEvent = 'focus_start' | 'focus_end' | 'focus_pause' | 'focus_resume' | 'rest_end' | 'all_done';
...
function genBeep(ctx: AudioContext) { beep(ctx, 800, 0.15); }
function genDoubleBeep(ctx: AudioContext) { beep(ctx, 660, 0.12, 0); beep(ctx, 880, 0.12, 0.15); }
function genChimeUp(ctx: AudioContext) { beep(ctx, 523, 0.2, 0); beep(ctx, 659, 0.2, 0.15); beep(ctx, 784, 0.3, 0.3); }
function genChimeDown(ctx: AudioContext) { beep(ctx, 784, 0.2, 0); beep(ctx, 659, 0.2, 0.15); beep(ctx, 523, 0.3, 0.3); }

/** 噔噔咚 — F# E C (descending) */
function genDengDengDong(ctx: AudioContext) { beep(ctx, 370, 0.25, 0); beep(ctx, 330, 0.25, 0.15); beep(ctx, 262, 0.35, 0.3); }
...
const GENERATORS: Record<string, (ctx: AudioContext) => void> = {
  beep: genBeep, double_beep: genDoubleBeep, chime_up: genChimeUp, chime_down: genChimeDown,
  dengdengdong: genDengDengDong, shibamata: genShibamata,
};
...
const DEFAULT_PREFS: SoundPrefs = {
  focus_start: 'chime_up', focus_end: 'chime_down',
  focus_pause: 'beep', focus_resume: 'beep',
  rest_end: 'double_beep', all_done: 'chime_up',
};
...
export class SoundManager {
  ...
  getSound(event: SoundEvent): string { return this.prefs[event]; }
  ...
  removeSound(key: string) {
    ...
    for (const ev of Object.keys(this.prefs) as SoundEvent[]) {
      if (this.prefs[ev] === key) { this.prefs[ev] = DEFAULT_PREFS[ev]; changed = true; }
    }
  ...
  private findSound(key: string): SoundDef | undefined {
    return BUILTIN_SOUNDS.find(s => s.key === key)

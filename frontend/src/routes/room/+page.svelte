<script lang="ts">
	...
	function phaseLabel(): string {
		...
		if (p === 'following')
			return `${t('following', $locale)} ${$pomodoroStatus.leader_username}`;
		if (p === 'rest')
			return $pomodoroStatus.is_long_break
				? t('rest_long', $locale)
				: t('rest_short', $locale);
			...
			{#if showSettings}
				...
				<div class="card settings-card">
					<div class="settings-header">
						...
						<button class="btn-ghost btn-sm" onclick={() => (showSettings = false)}
					...
					<div class="setting-row">
						<label class="toggle-label">
							<input
								...
								onchange={() => saveNotifyPref(notifyEnabled)}
							/>
								...
								<div class="sound-row-right">
									...
									>
										{#each allSounds as s (s.key)}
											<option
												value={s.key}
												selected={sound.getSound(ev) === s.key}
												>{s.label}</option
											>
										{/each}
									</select>
								...
								{#each allSounds.filter((s) => s.source !== 'builtin') as s (s.key)}
									<li>
										...
										<span
											><button
												class="btn-text btn-xs"
												onclick={() => sound.preview(s.key)}>▶</button
											>
											<button
												class="btn-text btn-xs"
												onclick={() => {
					...
					<div class="dialog-overlay" onclick={() => (showSoundDialog = false)}>
						<div class="dialog" onclick={(e) => e.stopPropagation()}>
							...
							<div class="dialog-actions">
								...
								>
								<button class="btn-secondary" onclick={addCustomSound}
									>{t('add', $locale)}</button
								>
			...
			{:else}
				...
				<div class="card timer-card">
					...
					<div class="timer-display">
						...
						<span
							class="status-label"
							class:focusing={$pomodoroStatus.phase === 'focusing' ||
								$pomodoroStatus.phase === 'following'}
					...
					{#if $pomodoroStatus.phase === 'idle'}
						...
						<button
							class="btn-secondary btn-lg btn-full"
							onclick={() => (showSettings = true)}
							style="margin-top:0.5rem">{t('settings', $locale)}</button
						>
					{:else if $pomodoroStatus.phase === 'paused'}
						<div class="timer-actions">
							<button class="btn-primary" onclick={handleResume}
								>▶ {t('continue', $locale)}</button
							>
							<button class="btn-ghost" onclick={handleStop}
								>⏹ {t('stop', $locale)}</button
							>
						</div>
					{:else if $pomodoroStatus.phase === 'rest'}
						<div class="timer-actions">
							<button class="btn-primary" onclick={handleSkip}
								>⏭ {t('skip', $locale)}</button
							>
						</div>
					{:else}
						<div class="timer-actions">
							<button class="btn-secondary" onclick={handlePause}
								>⏸ {t('pause', $locale)}</button
							>
							<button class="btn-secondary" onclick={handleEnd}
								>{t('end_pomodoro', $locale)}</button
							>
							<button class="btn-ghost" onclick={handleStop}
								>⏹ {t('stop', $locale)}</button
							>
							{#if $pomodoroStatus.phase === 'following'}
								<button class="btn-ghost" onclick={unfollowPomodoro}
									>{t('unfollow', $locale)}</button
								>
							{/if}
						</div>
					{/if}
								...
								<div class="user-info">
									<div class="user-details">
										<span class="user-name"
											>{user.username}
											{#if user.id === $currentMember?.id}<span
													class="you-badge">{t('me', $locale)}</span
												>{/if}
									...
									{#if user.pomodoro?.phase && user.pomodoro.phase !== 'idle'}
										{#if user.pomodoro.is_following}
											<span class="following"
												>🔄 {t('following', $locale)}
												{user.pomodoro.leader_username}</span
											>
										{:else if user.pomodoro.phase === 'paused'}
											<span class="paused-status"
												>⏸ {t('paused', $locale)}</span
											>
